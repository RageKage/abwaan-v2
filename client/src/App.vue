...
const handleOnline = () => { serverDown.value = false }
const handleOffline = () => { serverDown.value = true }
  ...
  >
    ...
    <transition name="slide-up">
      <div v-if="serverDown && !isDismissed" class="fixed bottom-6 left-4 right-4 z-[200] md:left-auto md:right-6 md:w-96">
        <div class="bg-white border-l-4 border-red-500 rounded-xl p-4 shadow-2xl flex items-start gap-4 ring-1 ring-black/5">
          ...
          <div class="flex-1">
            ...
            <button @click="PageReload" class="mt-3 text-xs font-bold text-red-600 bg-red-50 px-3 py-1.5 rounded-lg hover:bg-red-100 transition-colors">Retry Connection</button>
          </div>
