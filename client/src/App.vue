...
// Show nav everywhere except /login (matches your current behavior)
const showNav = computed(() => route.path !== '/login')

// Home page gets full-width (no padding shell)
const isHomePage = computed(
  () => route.path === '/' || route.name === 'Home' || route.name === 'home',
)

// Loading overlay (wire this to stores later; keep it here for now)
const isLoading = ref(false)

// "Server down" toast: for this Firebase-first app, use online/offline as the practical signal.
// (If you later add an API health-check, you can replace this without touching the template.)
...
// If connection comes back, allow the toast to show again next time
...
<template>
  <div
    id="app"
    class="bg-gray-50 min-h-screen text-gray-900 font-sans selection:bg-carrotOrange-200 selection:text-carrotOrange-900"
  >
    <TheNavigation v-if="showNav" class="max-w-[1500px] mx-auto" />
    ...
    <router-view v-slot="{ Component }">
      <transition name="page-fade" mode="out-in" appear>
        <component
          ...
          :class="[!isHomePage ? 'pt-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto' : 'pt-0 w-full']"
        />
      </transition>
    </router-view>
