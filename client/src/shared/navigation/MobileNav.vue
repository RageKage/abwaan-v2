...
<template>
  ...
  >
    <div
      v-if="isOpen"
      class="fixed inset-0 z-[100] flex flex-col bg-white text-gray-900 font-sans"
    >
      ...
      <header class="flex-shrink-0 h-20 flex items-center justify-between border-b border-gray-200 bg-white">
        ...
        >
          ...
          <div class="relative w-6 h-6">
            <span class="absolute top-1/2 left-0 w-full h-0.5 bg-current transform -translate-y-1/2 rotate-45 transition-transform group-hover:rotate-0"></span>
            <span class="absolute top-1/2 left-0 w-full h-0.5 bg-current transform -translate-y-1/2 -rotate-45 transition-transform group-hover:rotate-0"></span>
          </div>
        </button>
      </header>
      ...
      <div class="flex-1 overflow-y-auto">
        <nav class="flex flex-col">
          <router-link
            ...
            @click="handleClose"
          >
            <span class="text-3xl md:text-4xl font-serif text-gray-900 group-hover:text-carrotOrange-600 transition-colors">
              {{ route.label }}
            </span>

            <div class="flex items-center gap-4">
              <span class="font-mono text-xs text-gray-300 group-hover:text-carrotOrange-400">0{{ index + 1 }}</span>
              <span class="text-2xl text-gray-200 group-hover:text-carrotOrange-500 transition-colors transform group-hover:translate-x-2 duration-300">&rarr;</span>
            </div>
          </router-link>
        </nav>
      </div>
      ...
      <div class="flex-shrink-0 bg-gray-50 border-t border-gray-200 p-6 md:p-8">
        ...
        <div v-if="user" class="space-y-6">
            <div class="flex items-center gap-4">
                <div class="h-14 w-14 rounded-full border border-gray-300 bg-white flex items-center justify-center text-xl font-bold text-gray-900">
                  {{ user.displayName ? getInitial(user.displayName) : 'U' }}
                </div>
                <div>
                  <p class="font-bold uppercase tracking-widest text-sm text-gray-900">{{ user.displayName || 'Contributor' }}</p>
